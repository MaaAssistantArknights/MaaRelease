...
版本：v6.5.0 及以上
位置：小工具 → 小游戏 → 卫戍协议：盟约 下半

----

### 关于旧版本 MAA 无法使用的说明 (NEW!!!)

神秘鹰角在游戏数据中加入了 4 个 `code` 和 `name` 均为 `null` 的关卡。
由于旧版本未对这种情况做处理，**v6.4.2 之前的所有版本在运行时都会报资源损坏后崩溃**。  
edit: **v6.4.3 之前的所有版本在运行自动战斗时会崩溃**

因此本次基本等同于 **需要强制更新**，请尽快更新至最新版本。
...
同时，本次更新也整理了 自定义基建的轮换机制。现在自定义基建支持两种轮换模式：

- 根据配置文件中的时间自动轮换
- 每次执行完成后按顺序轮换到下一个配置

自定义基建的「计划选择」已迁移至新的轮换逻辑中。需要注意的是，如果在此前基建重构后从未手动切换过基建计划，旧配置可能会被识别为按顺序轮换（旧版本中只要存在时间配置就会默认按时间轮换）。若发现轮换行为与预期不符，手动重新选择一次基建计划或轮换方式即可恢复正常。
