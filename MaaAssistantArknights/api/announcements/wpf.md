...
MAA 现已支持 17 章关卡导航，两个产出新材料的关卡 17-5 与 17-11 也已经在关卡提示中放出，若有其他关卡的刷取需求可以使用「理智作战 - 高级设置 - 手动输入关卡名」 功能自行指定

但请注意，17 章的常规作战模式和险地作战模式的代理指挥不通用，因此您可能需要手动调整模式，确认打开关卡界面时（即右下角出现开始作战按钮时）代理指挥按钮可用，MAA 方可正常工作（仅需调整一次）
