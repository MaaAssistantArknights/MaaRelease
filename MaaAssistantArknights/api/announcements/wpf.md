...
v5.27.0-beta.1（即公测版当前版本）已适配界面主题「齐聚」，正式版用户请等待 v5.27.0 版本的发布

~~由于「促融共竞#2」 版本中上线的界面主题「齐聚」存在大量动态组件，该主题的适配和测试可能需要较长时间，各位想要正常使用 maa 的博士请在短期内尽量避免使用该主题，完成适配后我们将另行通知，感谢各位的理解和配合~~

----

### 5.26.2 及以下版本不支持界园肉鸽 DLC

5.26.2 版本主要是针对外服的更新，提交审核时间较早，在国服界园肉鸽 DLC 上线前已进入发布流程，因此 **5.26.2 暂未支持该 DLC 内容**。相关适配将在下个版本中提供。

科技树中有延长地图的节点，解锁后会将无法使用指挥分队快速存钱与烧水。  
新地图与新事件可能会无法识别从而放弃本次探索。  
放弃探索会卡在主界面，前往 `图册/见字` - `展演/戏剧` 中将 `界园志异` 设为活动主界面背景可以正常放弃探索。

----

### 5.26.1 以下版本无法使用自动公招

**游戏内公招 UI 已更改**，5.26.1 以下版本无法正常使用，如有需要请尽快更新以恢复公招功能。

----

### 16 章主线

16 章主线关卡有新增表现形式，部分关卡可能无法使用自动战斗。  
请关注后续版本更新，以便正常获取 16 章作业内容。
