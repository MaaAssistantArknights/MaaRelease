...
目前已发现有人冒充项目创建假仓库、篡改源码署名等行为，甚至在其中夹带恶意可执行文件——例如在系统临时目录（`%TEMP%`）释放隐蔽的可执行程序，用于窃取个人信息、植入木马等。  
这些恶意程序还会主动检测自身是否运行在虚拟机或调试工具中，一旦察觉就立刻闪退或自行删除，让安全分析人员难以抓取样本，普通用户更是难以察觉。  

我们已经针对已发现的仿冒仓库向 GitHub 提交了 DMCA 投诉，但由于处理流程原因，相关仓库目前仍可能被搜索到。

在此提醒大家：

• MAA 及相关项目完全免费、开源，**不会**以"关注、三连、私信"等方式作为获取资源的前提条件；  
• **官方唯一安全入口是[官网](https://maa.plus)**，所有下载链接、仓库地址均通过官网提供。搜索结果中可能存在同名仿冒仓库，请务必以官网链接为准；  
• 因部分地区 GitHub 下载较慢，我们会在官方用户群的群文件中同步上传最新版压缩包。官方群入口可在官网查看，或在软件设置 → 关于我们 → QQ 群中获取，请不要通过其他任何渠道加群；  
