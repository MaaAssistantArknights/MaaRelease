### v6.3.0 Beta 配置迁移说明公告 (NEW!!!)

此前发布的 v6.3.0-beta.1 与 v6.3.0-beta.2 中，少部分配置文件存在异常的用户可能会遇到配置迁移失败的问题，表现为进入主界面后任务列表为空。在 v6.3.0-beta.3 中已加入兼容处理并修复了该问题。

配置迁移仅会在「任务列表为空」的情况下触发。对于曾在迁移失败后手动添加过部分任务、但仍希望重新进行配置迁移的用户，可先移除任务列表中的所有任务并重启 MAA，以重新触发迁移流程。

在进行上述操作前，建议先备份 `config` 文件夹。如需还原配置，请在关闭 MAA 的情况下，将备份的 `config` 文件夹覆盖回原位置即可恢复。
