### 1 月 9 日游戏更新后无法使用自动战斗 (NEW!!!)

因《明日方舟》更新后暂停按钮位置调整，导致自动战斗等功能异常。我们已通过热更新修复此问题，请重启软件并留意右上角提示，即可恢复正常使用。

----

### 会客室卡线索交流与赠送

近期，国服基建会客室新增了「一键置入线索 / 一键赠送重复线索」功能，MAA v5.28.5 已支持。
由于个人线索上限为 10 张，且已放入会客室的线索不计入重复判断，提前放入过多线索可能导致到达上限却无法一键赠送或继续获取线索 ~~只能靠好友捞起来~~。

因此，MAA v6.1.0 调整为：仅在可一次性填满 7 个线索槽位时才执行一键置入。
若更新前会客室中已有线索，可能导致后续不再自动放入。

建议：先手动取下会客室中所有线索，再交由 MAA 全自动处理。

后续将优化为根据线索空位数量判断置入，但仍不建议手动放置过多线索，避免出现上述卡线索上限的情况。
