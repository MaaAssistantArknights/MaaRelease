### 关于 MuMu 模拟器焦点切换导致滑动中断的说明 (NEW!!!)

测试发现，MuMu 模拟器开启 ｢触控增强｣ 时，若在滑动操作过程中切换窗口或最小化模拟器，会导致滑动中断并产生明显的惯性漂移，该问题需由 MuMu 官方修复。为了避免该问题对基建换班、地图导航等需要滑动定位的场景造成影响，建议直接将模拟器保持最小化运行，或临时关闭 ｢触控增强｣ 模式使用其他触控方案。我们已将该问题反馈给 MuMu 团队，后续如有优化进展将同步告知。
